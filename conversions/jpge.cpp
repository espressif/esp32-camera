...
// v1.04a, Aug. 31, 2022 - Added optional two pass feature from  richgel999/jpeg-compressor
...
namespace jpge {
    ...
    static inline int left_shifti(int var, uint32 bits) { return static_cast<int> (static_cast<uint32>(var) << bits); }
    ...
    static uint32 m_huff_count[4][256];
...
#define JPGE_PUT_BYTE(c) { *m_pOut_buf++ = (c); if (--m_out_buf_left == 0) flush_output_buffer(); }
    ...
    {
        deinit();
        if (((!pStream) || (width < 1) || (height < 1)) || ((src_channels != 1) && (src_channels != 3) ) || (!comp_params.check())) return false; //&& (src_channels != 4)
        m_pStream = pStream;
