...
)
{
	...
	if (!hb || !hc || !hd || !jd->dptr || !jd->inbuf)
		return 0 - (int)JDR_FMT1;	/* Err: null data check */

	const uint16_t hc_len = jd->huffcode_len[id][0];
	const uint16_t* hc_last = hc + hc_len - 1;
...
JRESULT jd_prepare (
	...
	unsigned int (*infunc)(JDEC*, uint8_t*, size_t),	/* JPEG strem input function */
